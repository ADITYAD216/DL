...
class BaseNet(nn.Module):
    ...
    def generate_fc(self, in_dim, out_dim):
        hidden_dim = self.args.get("fcn_hidden_dim", 512)
        if self.args.get("fcn_type", "single") == "single":
            return SingleLinearLayer(in_dim, out_dim)
        elif self.args.get("fcn_type") == "mlp":
            return SmallMLP(in_dim, hidden_dim, out_dim)
        else:
            raise ValueError(f"Unsupported FCN type: {self.args.get('fcn_type')}")
...
class FCSIncrementalNet(BaseNet):
    ...
    def unset_gradcam_hook(self):
        if self._gradcam_hooks[0]:
            self._gradcam_hooks[0].remove()
        if self._gradcam_hooks[1]:
